...
def generate_token(usr_data):
    ...
    return encoded_access_token, generate_refresh_token(usr_data)

def generate_refresh_token(usr_data):
    refresh_expire = datetime.datetime.utcnow() + datetime.timedelta(seconds=refresh_token_expire_seconds)
    encode_refresh_token = jwt.encode({"exp":refresh_expire, **usr_data},rt_secret_key,algorithm="HS256")
    return encode_refresh_token


def validate_refresh_token(refresh_token):
    try:
        usrinfo = checkTokenValidation(refresh_token,rt_secret_key)
        usrinfo.pop('exp')
    except Exception as e:
        return JSONResponse(status_code=401, content={"error":str(e)})
    access_token, refresh_token = generate_token(usrinfo)
